...
export function ProjectMatrixView({
  ...
  const handleDeleteCol = (colName: string) => {
    if (window.confirm(`Tem a certeza que deseja eliminar o módulo "${colName}"? Os dados de TODAS as lojas serão perdidos.`)) {
      onDeleteColumn(project.id, colName);
    }
  };

  const handleEditHours = (rowId: string, currentHours?: number) => {
    setEditingHoursRowId(rowId);
    setEditHoursValue(currentHours ?? 0);
  };

  const saveHours = (rowId: string) => {
    onUpdateRowHours(project.id, rowId, store, Number(editHoursValue) || 0);
                   ...
                   <th key={col} style={{ 
                     ...
                     <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem', alignItems: 'center' }}>
                       ...
                       <div className="no-print" style={{ display: 'flex', gap: '0.4rem', borderTop: '1px solid #eee', marginTop: '0.25rem', paddingTop: '0.25rem' }}>
                          ...
                          <button onClick={() => handleDeleteCol(col)} style={{ background: 'transparent', border: 'none', color: '#fca5a5', fontSize: '0.7rem', cursor: 'pointer' }} title="Eliminar">🗑️</button>
                       </div>
                     </div>
                   </th>
                 ))
                   ...
                   <td style={{ padding: '0.75rem', border: '1px solid #e2e8f0', fontSize: '0.85rem', fontWeight: 600 }}>
                     <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '1rem' }}>
                       ...
                       <div className="no-print" style={{ display: 'flex', gap: '0.5rem' }}>
                         ...
                         <button 
                           onClick={() => onDeleteRow(project.id, row.id, store)}
